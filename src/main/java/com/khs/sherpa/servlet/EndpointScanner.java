...
public class EndpointScanner {
	...
	public void classPathScan(String pckg) {
		...
		if (url == null) {
			url = this.getClass().getClassLoader()
					.getResource("sherpa.properties");
		}

		if (url == null) {
			url = this.getClass().getClassLoader()
					.getResource(separator + "sherpa.properties");
		}
	...
	public void scanPackage(String pckg) {
		...
		for (String p : paths) {
			if (p.indexOf(".jar") < 0) {
				scanPath(p, p + "/" + pckg.replace('.', File.separatorChar),
						pckg.replace('.', File.separatorChar));
			}
		}
	}
	...
	private void scanPath(String classPath, String dir, String pckg) {
		...
		Map<String, Class<?>> endpointClasses = new HashMap<String, Class<?>>();
		...
		for (i = 0; i < files.length; i++) {
			...
			if (file.isDirectory() == true) {
				...
				try {
					...
					clazz = Class.forName(className.replace(File.separatorChar,
							'.').replace(".class", ""));
					// check for endpoint...
					...
					LOG.severe(errmsg("ERROR loading @Endpoint "
							+ file.getName()));
		...
		ReflectionCache.addTypes(endpointClasses);
